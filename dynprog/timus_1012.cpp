// Solution for http://acm.timus.ru/problem.aspx?space=1&num=1012 ("K-based numbers, version 2")
...
int main()
{
    ...
    for (int i = 2; i <= n; ++i)
        cache.push_back((cache[i-1] + cache[i-2]) * static_cast<big_uint<>::carrier_t>(k-1));
