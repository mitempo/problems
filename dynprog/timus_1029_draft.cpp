...
vector<int> shortest_path(uint input[][MAX_N], int m, int n)
{
    ...
    // floor.
    pair<uint,int> min_paths[MAX_M][MAX_N]; 
    ...
    for (int i = 1; i < m; ++i)
    {
        uint paths_within_prev_floor[MAX_N][MAX_N];
...
int main()
{
    ...
    for (uint i = 0; i < extent<decltype(ext_input)>::value; ++i)
        copy(input[i], input[i] + extent<decltype(input),1>::value, ext_input[i]);
    */
